...
def create_csrf(ip, content):
    conn = create_connection()
    try:
        print("Le ip : " + ip,flush=True)
        print("Le content : " + content,flush=True)

        cursor = conn.cursor()
        cursor.execute("INSERT INTO csrf (ip, content) VALUES (?, ?)", (ip, content))
        conn.commit()
...
def get_csrf(ip):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT content FROM csrf WHERE ip =?", (ip,))
        row = cursor.fetchone()
        return row
    except Error as e:
        print(e)

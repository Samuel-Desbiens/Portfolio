...
GANDALF_FLAG = 'FLAG-7777'
SARUMAN_FLAG = 'FLAG-1973'
    ...
    #For standard users
        ...
        <!DOCTYPE html>
            ...
            <form action="/showfaqmessages" method="get">
                <button type="submit" id="getFaqButton">Show FAQ Messages</button>
            </form>
...
def welcome():
    ...
    if get_user_role(cookie) == 1:
        ...
        if (userName == 'Pippin'):
            return html("Welcome " + userName + ". You have " + str(soldeObj['solde'][0]) + "$ At account number : " + str(accountNumber) +". Here's your flag : " + PIPPIN_FLAG , cookie)

        if (userName == 'Gandalf'):
            return html("Welcome " + userName + ". You have "   + str(soldeObj['solde'][0]) + "$ At account number : " + str(accountNumber) +". Here's your flag : " + GANDALF_FLAG , cookie)

        if (userName == 'Saruman' and int(str(soldeObj['solde'][0])) >= 1000):
            return html("Welcome " + userName + ". You have " + str(soldeObj['solde'][0]) + "$ At account number : " + str(accountNumber) +". Here's your flag : " + SARUMAN_FLAG , cookie)

        return html("Welcome " + userName + ". You have " + str(soldeObj['solde'][0]) + "$ At account number : " + str(accountNumber), cookie)
    else:
        return html("Welcome!")
...
def validateCSRFToken(ip, token, csrfToken ):
    data = {'csrfToken': csrfToken, 'ip': ip, 'token' : token}
    print("csrf front-en : " + csrfToken, flush=True)

    response = requests.get('http://' + BACK_END_IP + ':' + BACK_END_PORT + '/checkcsrf', json=data)

    obj = json.loads(response.content.decode('utf-8'))


    if obj['message'] == 'success':
        #if csrfToken == obj['success']:
        return True
    return False
...
def register():
    ...
    #Vérifie le token csrf avant de créer le user
    if validateCSRFToken(request.remote_addr, get_token(), csrfToken):
    ...
    return html("CSRF Error")

@app.route('/login', methods=['POST'])
def login():
    data = {'username': request.form['loginName'], 'password':request.form['loginPassword'], 'ip': request.remote_addr}
    ...
    if 'success' in obj:
        ...
        resp.set_cookie('token', obj['user'], samesite='Strict')
    elif 'error' in obj:
...
def passwordreset():
    ...
    email = decoded_token.get('email', decoded_token['email'])
    
    data = {'email':email}
    response = request.post('http://'+BACK_END_IP+':'+BACK_END_PORT+'/passwordreset',json = data)
    return build_response(response,get_token())
    
@app.route('/passwordrest',methods=['POST'])

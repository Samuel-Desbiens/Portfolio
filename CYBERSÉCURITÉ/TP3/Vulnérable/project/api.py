    ...
    if get_user_role(token) == -1:
        ...
        <body>
            ...
            <form action="/passwordreset" method = "post">
                ...
                <button type="submit">Password Reset Request</button>
            </form>
...
def welcome():
    ...
    if get_user_role(cookie) == 1:
        decoded_token =  jwt.decode(cookie, TOKEN_KEY, algorithms="HS256")
        ...
        if userName == 'Bilbo':
            return html("Welcome " + userName + ". You have " + str(soldeObj['solde'][0]) + "$ At account number : " + str(accountNumber) +". Here's your flag : " + BILBO_FLAG , cookie)
...
def passwordreset():
    ...
    response = requests.post('http://'+BACK_END_IP+':'+BACK_END_PORT+'/passwordreset',json = data)
    return build_response(response,get_token())
    
@app.route('/changepassword',methods=['POST'])
def changepassword():
    data = {'username':request.form['username'],'code': request.form['code'],'newPassword': request.form['newpassword']}
    response = requests.post('http://'+BACK_END_IP+':'+BACK_END_PORT+'/changepassword',json = data)
    return build_response(response,get_token())
#MOD

...
app.BaseRandomCode = random.randint(0,9999999999)
app.NumberOfCode = 0
...
def login():
    try:
        ...
        if username == 'Gandalf':
            encoded = jwt.encode({'User':success[1] , 'Password':success[2],'Solde': success[4], 'Account number': success[5], 'Role':success[6], 'Flag':"FLAG-426632555333"}, app.config['SECRET_KEY'], algorithm="HS256")
...
def password_reset():
    try:
        app.NumberOfCode= app.NumberOfCode + 1
        if app.BaseRandomCode+app.NumberOfCode >= 9999999999:
            app.BaseRandomCode = 1000
            app.NumberOfCode = 0
        data = request.get_json()
        username = data.get('username')
        success = dbManager.set_code(app.BaseRandomCode+app.NumberOfCode,username)
        #SI TWILLIO MARCHAIT
        """
        if success:
            data = request.get_json()
        ...
        """
        if success:
            return jsonify({'message_id':str(app.BaseRandomCode+app.NumberOfCode)})
        else:
            return None
    except Exception as e:
        print(traceback.format_exc())
